<template>
  <AuthenticatedLayout :user="$page.props.auth.user">
              ...
              <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  <tr v-for="dette in dettes.data" :key="dette.id">
                    <td class="px-6 py-4 whitespace-nowrap">{{ dette.client?.surname || 'N/A' }}</td>
                    <td class="px-6 py-4 whitespace-nowrap">{{ formatDate(dette.date) }}</td>
                    <td class="px-6 py-4 whitespace-nowrap">{{ formatMoney(dette.montant) }} FCFA</td>
                    <td class="px-6 py-4 whitespace-nowrap">{{ formatMoney(dette.montantDu) }} FCFA</td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm space-x-2">
                      <Link :href="`/boutiquier/dettes/${dette.id}`" class="text-blue-600 hover:text-blue-900">
                        Voir
                      </Link>
                      <button v-if="dette.status !== 'Solde'" @click="openPaiementModal(dette)" class="text-green-600 hover:text-green-900">
                        Paiement
                      </button>
                      <button @click="confirmDelete(dette)" class="text-red-600 hover:text-red-900">
                        Supprimer
                      </button>
                    </td>
                  </tr>
      ...
      <div class="p-8">
        ...
        <form @submit.prevent="createDette">
          <div class="space-y-6">
            <!-- Client Selection -->
            <div>
              <InputLabel for="client_id" value="Client *" class="text-base mb-2" />
              <select
                id="client_id"
                v-model="detteForm.client_id"
                class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/50 rounded-lg shadow-sm transition-all duration-200 hover:border-indigo-300 text-base py-2.5"
                required
              >
            ...
            <div>
              ...
              <!-- Article Lines -->
              <div v-for="(line, index) in detteForm.articles" :key="index" class="grid grid-cols-12 gap-3 mb-3 bg-gray-50 p-3 rounded-lg hover:bg-gray-100 transition-colors duration-200">
                <div class="col-span-5">
                  <select
                    v-model="line.article_id"
                    @change="onArticleSelect(index)"
                    class="block w-full border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/50 rounded-lg shadow-sm text-sm transition-all duration-200 hover:border-indigo-300 py-2"
                    required
                  >
                ...
                <div class="col-span-2 flex items-center justify-between">
                  <span class="text-sm font-semibold text-indigo-600">{{ formatMoney(line.quantite * line.prix) }}</span>
                  <button
            ...
            <!-- Total -->
            <div class="bg-gradient-to-r from-indigo-50 to-purple-50 p-5 rounded-lg border-2 border-indigo-200">
              <div class="flex justify-between items-center">
                <span class="text-lg font-semibold text-gray-700">Montant Total:</span>
          ...
          <div class="mt-8 flex justify-end space-x-3">
            ...
            <PrimaryButton type="submit" :disabled="detteForm.processing || detteForm.articles.length === 0">
              Créer la Dette
            </PrimaryButton>
          </div>
        </form>
    ...
    <Modal :show="showPaiementModal" @close="closePaiementModal" max-width="2xl">
      <div class="p-8">
        <h3 class="text-2xl font-bold mb-6 bg-gradient-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent">
        ...
        <form @submit.prevent="addPaiement">
          <div class="space-y-6">
            <!-- Dette Info -->
            <div class="bg-gradient-to-r from-gray-50 to-slate-50 p-5 rounded-lg border border-gray-200 space-y-3">
              <div class="flex items-center justify-between">
                <span class="text-sm font-medium text-gray-600">Client:</span>
                <span class="text-base font-semibold text-gray-900">{{ paiementForm.dette?.client?.surname }}</span>
              </div>
              <div class="flex items-center justify-between">
                <span class="text-sm font-medium text-gray-600">Montant total:</span>
                <span class="text-base font-semibold text-gray-900">{{ formatMoney(paiementForm.dette?.montant) }} FCFA</span>
              </div>
              <div class="flex items-center justify-between">
                <span class="text-sm font-medium text-gray-600">Déjà payé:</span>
                <span class="text-base font-semibold text-green-600">{{ formatMoney(paiementForm.dette?.montantDu) }} FCFA</span>
              </div>
              <div class="border-t border-gray-300 pt-3 flex items-center justify-between">
                <span class="text-base font-semibold text-gray-700">Reste à payer:</span>
                <span class="text-xl font-bold text-red-600">{{ formatMoney(paiementForm.dette?.montantRestant) }} FCFA</span>
              </div>
            </div>

            <!-- Montant Input -->
            <div>
              <InputLabel for="montant_paiement" value="Montant du paiement (FCFA) *" class="text-base mb-2" />
              <TextInput
                id="montant_paiement"
          ...
          <div class="mt-8 flex justify-end space-x-3">
            <SecondaryButton type="button" @click="closePaiementModal">Annuler</SecondaryButton>
            <PrimaryButton type="submit" :disabled="paiementForm.processing">
