  ...
  <div class="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-50">
    ...
    <nav class="bg-white/80 backdrop-blur-md border-b border-gray-200 shadow-sm sticky top-0 z-40">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
          ...
          <div class="flex md:hidden items-center">
            <button
              ...
              class="inline-flex items-center justify-center p-2 rounded-lg text-gray-600 hover:text-indigo-600 hover:bg-indigo-50 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-indigo-500 transition-all duration-150"
            >
        ...
        >
          <div v-show="showMobileMenu" class="md:hidden border-t border-gray-200 bg-white/95 backdrop-blur-sm">
            <div class="px-2 pt-2 pb-3 space-y-1">
              ...
              <template v-if="isBoutiquier || isAdmin">
                <Link
                  href="/boutiquier/clients"
                  :class="[
                    $page.url.startsWith('/boutiquier/clients')
                      ? 'bg-indigo-50 border-indigo-500 text-indigo-700'
                      : 'border-transparent text-gray-600 hover:bg-gray-50 hover:border-gray-300 hover:text-gray-800',
                    'block pl-3 pr-4 py-2 border-l-4 text-base font-medium transition-colors duration-150'
                  ]"
                ...
                <Link
                  href="/boutiquier/articles"
                  :class="[
                    $page.url.startsWith('/boutiquier/articles')
                      ? 'bg-indigo-50 border-indigo-500 text-indigo-700'
                      : 'border-transparent text-gray-600 hover:bg-gray-50 hover:border-gray-300 hover:text-gray-800',
                    'block pl-3 pr-4 py-2 border-l-4 text-base font-medium transition-colors duration-150'
                  ]"
                ...
                <Link
                  href="/boutiquier/dettes"
                  :class="[
                    $page.url.startsWith('/boutiquier/dettes')
                      ? 'bg-indigo-50 border-indigo-500 text-indigo-700'
                      : 'border-transparent text-gray-600 hover:bg-gray-50 hover:border-gray-300 hover:text-gray-800',
                    'block pl-3 pr-4 py-2 border-l-4 text-base font-medium transition-colors duration-150'
                  ]"
              ...
              <template v-if="isClient">
                <Link
                  href="/client/dettes"
                  :class="[
                    $page.url.startsWith('/client/dettes')
                      ? 'bg-indigo-50 border-indigo-500 text-indigo-700'
                      : 'border-transparent text-gray-600 hover:bg-gray-50 hover:border-gray-300 hover:text-gray-800',
                    'block pl-3 pr-4 py-2 border-l-4 text-base font-medium transition-colors duration-150'
                  ]"
              ...
              <div class="border-t border-gray-200 pt-3 mt-3">
                <div class="flex items-center px-3 py-2">
                  <div class="w-10 h-10 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-full flex items-center justify-center">
                    <span class="text-white font-semibold">{{ user.nom?.charAt(0) }}{{ user.prenom?.charAt(0) }}</span>
                  </div>
                  <div class="ml-3">
                    <p class="text-sm font-medium text-gray-900">{{ user.nom }} {{ user.prenom }}</p>
                    <p class="text-xs text-gray-500">{{ user.role?.libelle }}</p>
                  </div>
                </div>
                <Link
                  href="/logout"
                  method="post"
                  as="button"
                  class="block w-full text-left pl-3 pr-4 py-2 border-l-4 border-transparent text-base font-medium text-red-600 hover:bg-red-50 hover:border-red-300 transition-colors duration-150"
                >
                  <svg class="w-4 h-4 inline-block mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
                  </svg>
